...
After testing different data augmentation methods, we realized that elastic transforms resulted in the best outputs. The addition of elastic transforms seemed to reduce the Wasserstein distance, while maintaining similar stability. This suggests that the elastic transformations, which add deformations to the data, improved the robustness of the model, and forced it to learn more generalizable features as opposed to overfitting. The addition of Poisson noise significantly increased the Wasserstein distance during training. This was likely due to the distortions in the data making it difficult for our critic to differentiate real images from generated ones. This could possibly be mitigated with regularization techniques.

# Conclusion
**Summary**:
Classical GANs are producing extremely realistic results with QGANs still far away from those levels. However, QGANs are still a promising area of research. This paper shows a method of generating 28 x 28 pixel images of handwritten numbers. We successfully generated a three class subset of the MNIST dataset: 0/1/3. Even though classical GANs have been able to do this, we achieved this task with three orders of magnitude fewer trainable parameters in the generator. Furthermore, we found that by using elastic transforms, the Wasserstein distance was lowered globally and produced clearer images, especially for the 3's. However, poisson noise significanlty raised the Wasserstein distance to an average of around 45 while still producing almost similar outputs as an unmodified model.

**Impact**:
These findings further the promise of quantum machine learning by showing that it has huge potential against classical models with billions of parameters. If a quantum version could be created, it has the chance to decrease the resources needed by multiple magnitudes. Quantum machine learning could be the solution to problems unsolvable before due to a restriction of technological capabilities. Our findings show that imply that simple methods of data augmentation to training data can further improve performance.
